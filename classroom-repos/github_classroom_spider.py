...
class GithubClassroomSpider:
    ...
    def __init__(self, organization="sustech-cs304", repos_dir="./repos"):
        ...
        self.valid_extensions = ['.py', '.java', '.js', '.jsx', '.ts', '.tsx', '.c', '.cpp', '.h', '.html', '.css', '.md']
        self.single_file_insertion_limit = 2000  # 单文件插入行数阈值
        self._log("GithubClassroomSpider 初始化完成。")

    def _log(self, msg):
        print(f"[GithubClassroomSpider] {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} - {msg}")
    ...
    def _run_query(self, query, variables):
        response = requests.post(self.API_URL, json={"query": query, "variables": variables}, headers=self.HEADERS, timeout=15)
    ...
    def extract_team_name_from_repo(self, semestar_name, repo_name):
        if semestar_name == "25spring":
            match_res = re.match(r"team-project-25spring-(team[_-]?\d+|\d+)$", repo_name)
            if match_res:
                return match_res.group(1)
            else:
                return None
        elif semestar_name == "24spring":
            match_res = re.match(r"team-project-24spring-(team[_-]?\d+|\d+)$", repo_name)
            if match_res:
                return match_res.group(1)
            else:
                return None
    ...
    def get_project_assignment_id(self):
        def get_classrooms():
            ...
            page = 1
            while True:
                resp = requests.get(url, headers=self.request_headers, params={'page': page, 'per_page': 100}, timeout=15)
                if resp.status_code != 200:
                    self._log(f"Error fetching classrooms: {resp.status_code} - {resp.text}")
                    return []
                data = resp.json()
                if not data:
                    break
        ...
        def get_assignments(classroom_id):
            ...
            while True:
                resp = requests.get(url, headers=self.request_headers, params={'page': page, 'per_page': 100}, timeout=15)
                if resp.status_code != 200:
                    self._log(f"Error fetching assignments: {resp.status_code} - {resp.text}")
                    return []
                data = resp.json()
                if not data:
                    break
    ...
    def get_accepted_assignments(self, semestar_list=("23spring", "24spring", "25spring")):
        ...
        for semestar in semestar_list:
            ...
            if resp.status_code != 200:
                self._log(f"Error fetching accepted assignments: {resp.status_code} - {resp.text}")
                continue
            data = resp.json()
        ...
        for semestar, repo_members in self.tmp_data['group_members'].items():
            for repo_name, students in repo_members.items():
                for student in students:
                    ...
                    sucess_count += 1
        self._log(f"关联完成。成功关联 {sucess_count} 名同学，失败 {fail_count} 名同学。")
        self._save_json(self.tmp_path, self.tmp_data)
    ...
    def gather_data_from_local_repos(self):
        ...
        semestar_dirs = [d for d in os.listdir(self.repos_dir) if os.path.isdir(os.path.join(self.repos_dir, d))]
        all_results = {}
                    ...
                    for branch in repo.branches:
                        for commit in repo.iter_commits(branch):
                            if commit.hexsha not in all_commits:
                                ...
                                valid, _ = check_valid(semestar_name, commit.author.name, commit.committed_datetime.isoformat())
                                if not valid:
                                    continue
                                ...
                                for file_path, file_stat in stats.items():
                                    ...
                                    if file_stat.get('insertions', 0) > self.single_file_insertion_limit:
                                        continue
                                    # 删的太多也不行
                                    if file_stat.get("deletions", 0) > self.single_file_insertion_limit:
                                        continue
                                    _, ext = os.path.splitext(file_path)
                                    ext = ext.lower()
                                    if ext in self.valid_extensions:
                                        commit_file_stats[ext] += max(file_stat['insertions'] - file_stat['deletions'], 0)  # 别整成负数了
    ...
    def fetch_prs(self):
        ...
        def fetch_one_repo_pr(repo_name):
            ...
            cursor = None
            while has_next:
                variables = {"owner": self.organization, "repo": repo_name, "first": 50, "after": cursor}
                try:
        ...
        if self.MULTI_THREAD:
            try:
                with ThreadPoolExecutor(max_workers=self.WORKERS) as executor:
                    futures = {executor.submit(fetch_one_repo_pr, repo): repo for repo in all_repos}
    ...
    def fetch_issues(self):
        ...
        if self.MULTI_THREAD:
            try:
                with ThreadPoolExecutor(max_workers=self.WORKERS) as executor:
                    futures = {executor.submit(fetch_one_repo_issue, repo): repo for repo in all_repos}
                    for future in as_completed(futures):
                        repo = futures[future]
                        try:
    ...
    def fetch_branches(self):
        ...
        if self.MULTI_THREAD:
            try:
                with ThreadPoolExecutor(max_workers=self.WORKERS) as executor:
                    futures = {executor.submit(fetch_one_repo_branch, repo): repo for repo in all_repos}
                    for future in as_completed(futures):
                        repo = futures[future]
                        try:
    ...
    def fetch_commit_authors(self):
        ...
        def process_repo(repo_name, commit_shas):
            self._log(f"开始爬取 {repo_name} 的提交作者信息 - {len(commit_shas)} 个提交")
            users = []
            if self.MULTI_THREAD:
                with ThreadPoolExecutor(max_workers=self.WORKERS) as executor:
                    futures = {
    ...
    def filter_commits_by_classroom_user(self):
        ...
        for semestar, repos in self.tmp_data.get('local_data', {}).items():
            ...
            for repo in repos:
                remote_commitor_info = self.tmp_data['commit_authors'].get(repo['repo_name'], [])
                email2id = {c['email']: c['id'] for c in remote_commitor_info}
                group_members = self.tmp_data['group_members'].get(semestar, {}).get(repo['repo_name'], [])
                valid_ids = set(p['github_id'] for p in group_members)
                valid_commits = []
                for commit in repo['commits']:
                    if commit["author_email"] not in email2id:  # 鉴定为外部commitor
                        continue
                    if email2id[commit['author_email']] in valid_ids:  # 鉴定为小组内成员
                ...
                self.tmp_data['filtered_local_data'][semestar].append({
                    ...
                    'group_member_ids': [val for val in valid_ids if val is not None]
                })
        self._save_json(self.tmp_path, self.tmp_data)
    ...
    def generate_chart_data(self):
        ...
        for semestar, repos in self.tmp_data.get('filtered_local_data', {}).items():
            ...
            # 4. 代码行数分布
            code_line_counts = [repo.get('code_line_data', {}).get('total_lines', 0) for repo in repos]
            avg_lines = sum(code_line_counts) / len(code_line_counts) if code_line_counts else 0
            ...
            # 6. PR数/Issue数/分支数
            pr_counts = [len(self.tmp_data.get('pr', {}).get(repo['repo_name'], [])) for repo in repos]
            issue_counts = [len(self.tmp_data.get('issues', {}).get(repo['repo_name'], [])) for repo in repos]
            branch_counts = [len(self.tmp_data.get('branches', {}).get(repo['repo_name'], [])) for repo in repos]
            ...
            chart_data[semestar]['branch_count_per_repo'] = {
                ...
                'average_branches': sum(branch_counts)/len(branch_counts) if branch_counts else 0
            }
            # 7. PR状态分布
            pr_status_counter = {
                'merged': 0,
            ...
            for repo in repos:
                ...
                for commit in repo['commits']:
                    ...
                    change_lines_per_people[author_id] += commit.get('insertions', 0) + commit.get('deletions', 0)
    ...
    def auto_run(self):
        ...
        """
        self._log("======= 一键自动开始全流程 =======")
        self.get_accepted_assignments()  # 获取作业的提交信息
        self.gather_data_from_local_repos()  # 本地仓库数据
        self.fetch_prs()  # PR
        self.fetch_issues()  # Issue
        self.fetch_branches()  # 分支
        self.fetch_commit_authors()  # 提交作者信息
        self.filter_commits_by_classroom_user()  # 过滤
        self.generate_chart_data()  # 图表
        self.save_all()  # 保存
        self._log("======= 全流程自动完成 =======")
